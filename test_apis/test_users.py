...
class TestUsersEndPoints(object):
    ...
    def test_with_all_correct_parameters(self):
        ...
        file_pointer.write('Some data will be different for obvious reasons\n'.format(
            datetime=datetime.now(), name=self.name))
        POST_DATA_FOR_USER['email'] = "s"+str(random.random())+"@gmail.com"
        response_php = requests.post(PHP_LINK + self.url, auth=self.http_auth,
                                      headers=self.header, data=POST_DATA_FOR_USER)
        POST_DATA_FOR_USER['email'] = "s" + str ( random.random () ) + "@yahoo.com"
