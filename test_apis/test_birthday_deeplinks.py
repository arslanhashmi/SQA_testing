...
class TestBirthdayDeeplinksEndPoints(object):
    name = 'birthday_deeplinks'
    url = BIRTHDAY_DEEPLINK
    ua = UserAgent()
        ...
        try:
            ...
            file_pointer.write(
                'While comparing JSON an exception occurred: {exception}\n'.format(
                    exception=e))
        file_pointer.write(
            'Test ends for {name} with correct date at {datetime}'.format(datetime=datetime.now(), name=self.name))
        assert True
        file_pointer.close()
