...
class TestSharingEndPoints(object):
    ...
    def test_sending_with_all_correct_parameters(self):
        ...
        file_pointer.write('Going to test {name} with correct date at {datetime}\n'.format(datetime=datetime.now(), name=self.name_send))

        response_php = requests.get(PHP_LINK + self.url_send, auth=self.http_auth,
                                      headers=self.header, )
        response_python = requests.get(PYTHON_LINK + self.url_send, auth=self.http_auth,
                                         headers=self.header,)
        ...
        try:
            ...
            file_pointer.write(
                'While comparing JSON an exception occured: {exception}\n'.format(
                    exception=e))
        assert True
        file_pointer.write('Test ends for {name} at {datetime}'.format(datetime=datetime.now(), name=self.name_send))
        file_pointer.close()
    ...
    def test_receiving_with_all_correct_parameters(self):
        ...
        response_php = requests.get(PHP_LINK + self.url_receive, auth=self.http_auth,
                                      headers=self.header, )
        response_python = requests.get(PYTHON_LINK + self.url_receive, auth=self.http_auth,
                                         headers=self.header,)
        ...
        try:
            ...
            file_pointer.write(
                'While comparing JSON an exception occured: {exception}\n'.format(
                    exception=e))
        assert True
        file_pointer.write('Test ends for {name} at {datetime}'.format(datetime=datetime.now(), name=self.name_receive))
        file_pointer.close()
