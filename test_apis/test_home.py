...
class TestHomeEndPoints(object):
    name = 'home'
    url = HOME
    ua = UserAgent()
        ...
        try:
            ...
            file_pointer.write(
                'While comparing JSON an exception occured: {exception}\n'.format(
                    exception=e))
        file_pointer.write('Test ends for outlets with correct date at {datetime}'.format(datetime=datetime.now()))
        file_pointer.close()

    def test_with_missing_arguments(self):
        """
        Testing by missing parameters one by one
        :return:
        """
        file_pointer = open('logs/{name}_less_keys_{date}.txt'.format(date=self.date, name=self.name), 'w')
        for parameter in KEYS_HOME:
            updated_link = HOME_WITHOUT_PARMS
            ...
            skip_param = [parameter]
            for parm, value in PARAMS_HOME.items():
                if parm not in skip_param:
            ...
            file_pointer.write('Test ends for {name} with skiping {parameter} at {datetime}\n\n'.format(
                datetime=datetime.now(), parameter=parameter, name=self.name))
        assert True # as we don't want to break the tests while skipping parameters
        file_pointer.close()
